...
public class Cipher {
	 ...
	 * 
	 * */
	/*ENCRYPT AND DECRYPT BYTE BLOCK FUNCTION
	* dont call this function unless you have initialized the stateblock.
	* */
	public byte[] encrypt_Block(){
		//key round is 0, add the round key
		this.add_Round_Key(this.keySpace.get(0));
		for(int i = 1; i < 11; i++){
			encrypt_Round(i);
		}
		return this.outBlkAsBytes();
	}
	public byte[] decrypt_Block(){
		//start of decyrypt, add key round 10
		this.add_Round_Key(this.keySpace.get(10));

		//decrypt the block
		for(int i = 9; i > -1; i--){
			this.decrypt_Round(i);
		}
		return this.outBlkAsBytes();
	}
	public void encrypt() {
		...
		while(tempPlain.length() > 0) {
			...
			//encrypt the block
			for(int i = 1; i < 11; i++) {
				this.encrypt_Round(i);
			}
			//write the block to cipher text
			ciph  += this.out_stateBlock();
		}
		this.set_Ciphertext(ciph);

		this.set_Plaintext("");
	}
	//encrypt round helper
	private void encrypt_Round(int round) {
	...
	public void decrypt() {
		...
		this.set_Plaintext(msg);
	}
	
	public void decrypt_Round(int round) {
...
//get the current block state
	public byte[] outBlkAsBytes(){
		byte[] outBlock = new byte[16];
		int cnt = 0;
		for(int i = 0; i < this.stateBlock.length; i++){
			for(int k = 0; k < this.stateBlock[k].length; k++){
				outBlock[cnt] = (byte)this.stateBlock[k][i];
			}
		}
		this.stateBlock = new int[4][4];
		return outBlock;
	}
	//write block from bytes
	public void init_StateBlock(byte[] bytes){
		...
		if (bytes.length == 0 || bytes == null) {
			...
			for(int i = 0; i < this.stateBlock.length; i++){
				for(int k = 0; k < this.stateBlock[i].length; k++){
					if(cnt <  bytes.length){
						this.stateBlock[k][i] = bytes[cnt];
						cnt++;
					}else{
						this.stateBlock[k][i] = 0b00000000;
					}
				}
			}
		}
	...
	public void init_StateBlock(String bytes) {
		if(bytes.length() == 0 || bytes == null) {
			return;
		}else {
			int cnt = 0;
	...
	//matrix operations using math
	private int matrix_GF_Ops(int[] scalarRow, int[] stateColumn){
		for(int i = 0; i < scalarRow.length; i++){
			if(scalarRow[i] == 2){
				stateColumn[i] = this.mod_Prime_Poly(stateColumn[i] << 1);
			}
			if(scalarRow[i] == 3){
				stateColumn[i] = this.mod_Prime_Poly((stateColumn[i] <<1)^stateColumn[i]);
			}
			if(scalarRow[i] == 9){
	...
	private int mod_Prime_Poly(int value) {
		if(value >= 256) {
			return this.mod_Prime_Poly(value^283);
		}
		else {
			return value;
		}
	...
	public void set_Plainbytes(byte[] plainbytes){
		this.plainbytes = plainbytes;
	}
	public byte[] get_PlainBytes(){
		return this.plainbytes;
	}
	public void set_Cipherbytes(byte[] cipherbytes){
		this.cipherbytes = cipherbytes;
	}
	public byte[] get_Cipherbytes(){
